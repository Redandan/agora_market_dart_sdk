...
class AuthPasskeyApi {
  ...
  /// Get current user's Passkey binding summary without account enumeration
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> securitySummaryWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/auth/passkey/security-summary';
  ...
  /// Get current user's Passkey binding summary without account enumeration
  Future<PasskeySecuritySummaryResponse?> securitySummary() async {
    final response = await securitySummaryWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
