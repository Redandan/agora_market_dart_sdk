...
class TestApi {
  ...
  /// Performs an HTTP 'GET /test/jwt-config' operation and returns the [Response].
  Future<Response> getJwtConfigWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/test/jwt-config';
  ...
  Future<Map<String, Object>?> getJwtConfig() async {
    final response = await getJwtConfigWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
