...
class AdminOcoControllerApi {
  ...
  /// Performs an HTTP 'POST /admin/oco/market-sell/{currency}/{qty}' operation and returns the [Response].
  /// Parameters:
  ///
  /// * [String] currency (required):
  ///
  ...
  ///
  /// * [bool] dryRun:
  Future<Response> marketSellQtyWithHttpInfo(String currency, num qty, { String? reason, bool? dryRun, }) async {
    // ignore: prefer_const_declarations
    final path = r'/admin/oco/market-sell/{currency}/{qty}'
      .replaceAll('{currency}', currency)
  ...
  ///
  /// * [bool] dryRun:
  Future<ApiResponseMapStringObject?> marketSellQty(String currency, num qty, { String? reason, bool? dryRun, }) async {
    final response = await marketSellQtyWithHttpInfo(currency, qty,  reason: reason, dryRun: dryRun, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
