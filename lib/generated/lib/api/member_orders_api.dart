...
class MemberOrdersApi {
  ...
  /// * [String] reason:
  ///   取消理由
  Future<Response> cancelDigitalOrderWithHttpInfo(String orderId, { String? reason, }) async {
    // ignore: prefer_const_declarations
    final path = r'/orders/{orderId}/cancel-digital'
      .replaceAll('{orderId}', orderId);
  ...
  /// * [String] reason:
  ///   取消理由
  Future<Order?> cancelDigitalOrder(String orderId, { String? reason, }) async {
    final response = await cancelDigitalOrderWithHttpInfo(orderId,  reason: reason, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
