...
class AdminDisputesApi {
  ...
  ///
  /// * [String] comment:
  Future<Response> denyAppealWithHttpInfo(String disputeId, { String? comment, }) async {
    // ignore: prefer_const_declarations
    final path = r'/admin/disputes/{disputeId}/deny-appeal'
      .replaceAll('{disputeId}', disputeId);
  ...
  ///
  /// * [String] comment:
  Future<Dispute?> denyAppeal(String disputeId, { String? comment, }) async {
    final response = await denyAppealWithHttpInfo(disputeId,  comment: comment, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  ///
  /// * [DisputeJudgmentParam] disputeJudgmentParam (required):
  Future<Dispute?> judgeDispute(String disputeId, DisputeJudgmentParam disputeJudgmentParam,) async {
  ...
  ///
  /// * [String] comment:
  Future<Response> reexamineDisputeWithHttpInfo(String disputeId, { String? comment, }) async {
    // ignore: prefer_const_declarations
    final path = r'/admin/disputes/{disputeId}/reexamine'
      .replaceAll('{disputeId}', disputeId);
  ...
  ///
  /// * [String] comment:
  Future<Dispute?> reexamineDispute(String disputeId, { String? comment, }) async {
    final response = await reexamineDisputeWithHttpInfo(disputeId,  comment: comment, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
