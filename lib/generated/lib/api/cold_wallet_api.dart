...
class ColdWalletApi {
  ...
  /// Parameters:
  ///
  /// * [ColdWallet] coldWallet (required):
  Future<Response> createColdWalletWithHttpInfo(ColdWallet coldWallet,) async {
    // ignore: prefer_const_declarations
    final path = r'/cold-wallet';

    // ignore: prefer_final_locals
    Object? postBody = coldWallet;
  ...
  Future<ColdWallet?> createColdWallet(ColdWallet coldWallet,) async {
    final response = await createColdWalletWithHttpInfo(coldWallet,);
  ...
  ///
  /// * [String] operator_:
  Future<Response> releaseAllocatedAmountWithHttpInfo(int id, num amount, String orderId, { String? operator_, }) async {
    // ignore: prefer_const_declarations
    final path = r'/cold-wallet/{id}/release-amount'
  ...
  ///
  /// * [String] operator_:
  Future<void> releaseAllocatedAmount(int id, num amount, String orderId, { String? operator_, }) async {
    final response = await releaseAllocatedAmountWithHttpInfo(id, amount, orderId,  operator_: operator_, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }
  ...
  ///
  /// * [String] orderId:
  Future<Response> testAddressAllocation1WithHttpInfo(String protocolEnum, num amount, String currency, { String? orderId, }) async {
    // ignore: prefer_const_declarations
    final path = r'/cold-wallet/allocate-test';
  ...
  Future<AddressAllocationResult?> testAddressAllocation1(String protocolEnum, num amount, String currency, { String? orderId, }) async {
    final response = await testAddressAllocation1WithHttpInfo(protocolEnum, amount, currency,  orderId: orderId, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'AddressAllocationResult',) as AddressAllocationResult;
  ...
  /// * [int] id (required):
  ///
  /// * [num] maxAmount (required):
  ///
  /// * [String] operator_:
  Future<Response> updateMaxRechargeAmountWithHttpInfo(int id, num maxAmount, { String? operator_, }) async {
    // ignore: prefer_const_declarations
    final path = r'/cold-wallet/{id}/max-amount'
      .replaceAll('{id}', id.toString());
  ...
  ///
  /// * [String] operator_:
  Future<void> updateMaxRechargeAmount(int id, num maxAmount, { String? operator_, }) async {
    final response = await updateMaxRechargeAmountWithHttpInfo(id, maxAmount,  operator_: operator_, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }
