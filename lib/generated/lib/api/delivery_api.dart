...
class DeliveryApi {
  ...
  Future<PageDeliveryDetail?> getDeliveryHistory(DeliveryHistoryParam deliveryHistoryParam,) async {
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeWithAsync(await _decodeBodyBytes(response), (dynamic value) => PageDeliveryDetail.fromJson(value)) as PageDeliveryDetail;
  ...
  Future<DeliveryerStatsDTO?> getDeliveryerStats() async {
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeWithAsync(await _decodeBodyBytes(response), (dynamic value) => DeliveryerStatsDTO.fromJson(value)) as DeliveryerStatsDTO;
  ...
  Future<Response> updateDeliveryer1WithHttpInfo(DeliveryerUpdateParam deliveryerUpdateParam,) async {
    // ignore: prefer_const_declarations
  ...
  ///
  /// * [DeliveryerUpdateParam] deliveryerUpdateParam (required):
  Future<Deliveryer?> updateDeliveryer1(DeliveryerUpdateParam deliveryerUpdateParam,) async {
    final response = await updateDeliveryer1WithHttpInfo(deliveryerUpdateParam,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
