...
class AdminNotificationsApi {
  ...
  /// * [int] size:
  ///   每頁數量
  Future<Response> searchNotifications1WithHttpInfo({ int? id, int? userId, int? senderId, String? title, String? type, String? status, bool? isPushed, String? relatedObjectId, String? relatedObjectType, DateTime? startDate, DateTime? endDate, int? page, int? size, }) async {
    // ignore: prefer_const_declarations
  ...
  Future<PageNotificationResponseDTO?> searchNotifications1({ int? id, int? userId, int? senderId, String? title, String? type, String? status, bool? isPushed, String? relatedObjectId, String? relatedObjectType, DateTime? startDate, DateTime? endDate, int? page, int? size, }) async {
    final response = await searchNotifications1WithHttpInfo( id: id, userId: userId, senderId: senderId, title: title, type: type, status: status, isPushed: isPushed, relatedObjectId: relatedObjectId, relatedObjectType: relatedObjectType, startDate: startDate, endDate: endDate, page: page, size: size, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
