...
class MemberDisputesApi {
  ...
  /// * [String] disputeId (required):
  ///
  /// * [DisputeAppealParam] disputeAppealParam (required):
  Future<Response> requestAppealWithHttpInfo(String disputeId, DisputeAppealParam disputeAppealParam,) async {
    // ignore: prefer_const_declarations
    final path = r'/disputes/{disputeId}/appeal'
      .replaceAll('{disputeId}', disputeId);

    // ignore: prefer_final_locals
    Object? postBody = disputeAppealParam;
  ...
  ///
  /// * [DisputeAppealParam] disputeAppealParam (required):
  Future<Dispute?> requestAppeal(String disputeId, DisputeAppealParam disputeAppealParam,) async {
    final response = await requestAppealWithHttpInfo(disputeId, disputeAppealParam,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
