...
class UserPostStatistics {
  /// Returns a new [UserPostStatistics] instance.
  UserPostStatistics({
    this.totalPosts,
    this.publishedPosts,
    this.draftPosts,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is UserPostStatistics &&
  ...
  @override
  String toString() => 'UserPostStatistics[totalPosts=$totalPosts, publishedPosts=$publishedPosts, draftPosts=$draftPosts, archivedPosts=$archivedPosts, totalViews=$totalViews, totalLikes=$totalLikes, totalComments=$totalComments, totalShares=$totalShares]';
  ...
  /// Returns a new [UserPostStatistics] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static UserPostStatistics? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "UserPostStatistics[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "UserPostStatistics[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return UserPostStatistics(
  ...
  static List<UserPostStatistics> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <UserPostStatistics>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = UserPostStatistics.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, UserPostStatistics> mapFromJson(dynamic json) {
    final map = <String, UserPostStatistics>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = UserPostStatistics.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of UserPostStatistics-objects as value to a dart map
  static Map<String, List<UserPostStatistics>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<UserPostStatistics>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = UserPostStatistics.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
