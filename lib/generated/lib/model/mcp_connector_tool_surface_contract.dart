...
class McpConnectorToolSurfaceContract {
    ...
    if (value is Map) {
      ...
      return McpConnectorToolSurfaceContract(
        ...
        requiredScopesByWorkflow: json[r'requiredScopesByWorkflow'] == null
            ? const {}
            : _mapStringList(json[r'requiredScopesByWorkflow']),
  ...
  static Map<String, List<String>> _mapStringList(dynamic value) {
    if (value is! Map) return const {};

    return value.map<String, List<String>>((key, listValue) {
      final list = listValue is Iterable
          ? listValue.map((item) => item.toString()).toList(growable: false)
          : const <String>[];
